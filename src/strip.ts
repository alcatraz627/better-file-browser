...
import { icoFile } from './icons';
import { getSaved, saveSaved } from './storage';
import { upsertPlace, removePlace } from './places';
import {
  EMPTY, openTab, closeTab, closeOthers, activate, togglePin, step, moveTab, labelFor, kindOf,
...
function readSession(): { sid: string; state: TabState } | null {
  try {
...
function writeSession(sid: string, state: TabState): void {
  try { sessionStorage.setItem(SESSION_KEY, JSON.stringify({ sid, state })); } catch { /* storage full or disabled */ }
}
